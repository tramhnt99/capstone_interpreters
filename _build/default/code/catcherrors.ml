...
(*
Lambda Calculus Propagating Errors Intepreter
...
let rec eval (e: exp) : value =
  ...
  | Binop (b, e1, e2) ->
     begin
       ...
       | (ErrorV s1, _) -> ErrorV s1
       | (_, ErrorV s2) -> ErrorV s2
       | _ -> ErrorV "e1 and/or e2 are not integers"
     end
  | Var _ -> ErrorV "eval free variable"
  ...
  | App (e1, e2) ->
     begin
       ...
       | (ErrorV s1, _) -> ErrorV s1
       | (_, ErrorV s2) -> ErrorV s2
       | _ -> ErrorV "tried to apply non-function"
     end
